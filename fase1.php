...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE canvas SET segmentos=%s, `created`=%s, modified=%s WHERE idcanvas=%s",
                       GetSQLValueString($_POST['segmentos'], "text"),
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
  $updateSQL = sprintf("UPDATE imagenes_plan_canvas SET imagen1=%s WHERE idimgcanvas=%s",
...
$varUsuario_consultaCanvas = "0";
if (isset($_SESSION["MM_idusuario"])) {
  $varUsuario_consultaCanvas = $_SESSION["MM_idusuario"];
}
mysql_select_db($database_crenesoft, $crenesoft);
$query_consultaCanvas = sprintf("SELECT * FROM canvas WHERE canvas.idcanvas = %s", GetSQLValueString($varUsuario_consultaCanvas, "int"));
$consultaCanvas = mysql_query($query_consultaCanvas, $crenesoft) or die(mysql_error());
$row_consultaCanvas = mysql_fetch_assoc($consultaCanvas);
$totalRows_consultaCanvas = mysql_num_rows($consultaCanvas);
?>
<!doctype html>
...
<title>Segmentos de clientes</title>
<!-- InstanceEndEditable -->
...
<?php } else{?>
    ...
    <section id="principal">
	  ...
	  remote = open('gestionimagen_plan_canvas1.php', 'remote', 'width=400, height=150, location=no, scrollbars=yes, menubars=no, resizable=yes, fullscreen=no, status=yes'); //propieades de las imagenes
	  remote.focus(); // esta ventanda que se abio pase a primer plano
	  }
  
  </script>
<fieldset class="well">
		<legend>Segmentos de clientes</legend>
        <p align="justify"><h4> Vale la pena dedicarle un rato a este punto. Permíteme un ejemplo absurdo: no aceptes que te digan “Yo hago comida para perros y por lo tanto mis clientes son todos los perritos en la ciudad”, ¡porque los perritos no toman la decisión de compra! Serán los consumidores finales del producto, pero tu cliente es esencialmente el dueño del perro ¿Estamos?</h4></p>

 <p align="justify"><h4>Identificar a qué clientes atender no es una tarea sencilla, pero si no lo haces tu proyecto no vivirá mucho tiempo, pues probablemente no atenderás bien a las personas u organizaciones que realmente están dispuestas a pagar más por tus productos o servicios. En este punto, te recomiendo que hagas una lluvia de ideas sobre las personas u organizaciones a las que te gustaría atender y cómo puedes agruparlos según sus problemas, necesidades y comportamientos.</h4></p>

<p align="justify"><h4>Lo anterior es la idea básica del proceso de Segmentación de Mercados. En algunas ocasiones te ayudará a darte cuenta de que hay un grupo o grupos desatendidos en el mercado y ahí puede haber una oportunidad de negocio, pero a veces te permitirá darte cuenta que hay un motivo por el cual nadie está atendiéndolos y que no te conviene meterte ahí. Entonces, explota la discusión en equipo porque pueden surgir muy buenas ideas si todos intentan ponerse en los zapatos de varios prospectos de cliente.</h4></p>
<p class="alert alert-warning">http://www.slideshare.net/VickyWatson2/metodologa-canvas-cmo-innovar-modelos-de-negocios-utilizando-solo-una-hoja </p>
       	  <div id="wrap">
          <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
            <table align="center">
              <tr valign="baseline">
                <td><textarea name="segmentos" style="width:190%" cols="50" rows="10.5"><?php echo htmlentities($row_consultaCanvas['segmentos'], ENT_COMPAT, 'utf-8'); ?></textarea></td>
              </tr>
              <tr valign="baseline">
                <td><input type="submit" class="btn btn-primary" value="Actualizar registro"></td>
              </tr>
            </table>
            <input type="hidden" name="created" value="<?php echo htmlentities($row_consultaCanvas['created'], ENT_COMPAT, 'utf-8'); ?>">
            <input type="hidden" name="modified" value="<?php echo htmlentities($row_consultaCanvas['modified'], ENT_COMPAT, 'utf-8'); ?>">
            <input type="hidden" name="MM_update" value="form1">
            <input type="hidden" name="idcanvas" value="<?php echo $row_consultaCanvas['idcanvas']; ?>">
          </form>
<fieldset class="well">
  <legend align="center">¡Selecciona una imagen para  esta etapa!</legend>
  ...
  <table align="center" cellpadding="4" cellspacing="4">
    <tr valign="baseline">
      <td rowspan="2" align="right" nowrap><img src="imagenes/plan_canvas/<?php echo $row_imageness['imagen1']; ?>" width="190" height="100"></td>
...
mysql_free_result($consultaCanvas);
?>
