...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE planes SET nombre_uno=%s, nombre_dos=%s, nombre_tres=%s WHERE idplan=%s",
                       GetSQLValueString($_POST['nombre_uno'], "text"),
                       GetSQLValueString($_POST['nombre_dos'], "text"),
                       GetSQLValueString($_POST['nombre_tres'], "text"),
  ...
  $updateGoTo = "plan_de_negocios_normal_acabado.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$varUsuario_datosnombrestres = "0";
if (isset($_SESSION["MM_idusuario"])) {
  $varUsuario_datosnombrestres = $_SESSION["MM_idusuario"];
}
mysql_select_db($database_crenesoft, $crenesoft);
$query_datosnombrestres = sprintf("SELECT * FROM planes WHERE planes.idplan = %s", GetSQLValueString($varUsuario_datosnombrestres, "int"));
$datosnombrestres = mysql_query($query_datosnombrestres, $crenesoft) or die(mysql_error());
$row_datosnombrestres = mysql_fetch_assoc($datosnombrestres);
$totalRows_datosnombrestres = mysql_num_rows($datosnombrestres);
?>
<!doctype html>
...
<?php } else{?>
    ...
    <section id="principal">
       ...
       <fieldset class="well">
      <legend align="center">¡Finalización!</legend>
		...
		<fieldset class="well">
			...
			<!--==============================================================-->
			<legend align="center">Nombre de integrante</legend>
			<div align="center" class="span4">
        <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
          <table align="center">
            <tr valign="baseline">
              <td><input type="text" required placeholder="Escribe el nombre del autor" title="ej: Giovanny Hidalgo Suarez" name="nombre_uno" value="<?php echo htmlentities($row_datosnombrestres['nombre_uno'], ENT_COMPAT, 'utf-8'); ?>" size="32"></td>
            </tr>
            <tr valign="baseline">
              <td><input type="text" required placeholder="Escribe el nombre del autor" title="ej: Giovanny Hidalgo Suarez" name="nombre_dos" value="<?php echo htmlentities($row_datosnombrestres['nombre_dos'], ENT_COMPAT, 'utf-8'); ?>" size="32"></td>
            </tr>
            <tr valign="baseline">
              <td><input type="text" required placeholder="Escribe el nombre del autor" title="ej: Giovanny Hidalgo Suarez" name="nombre_tres" value="<?php echo htmlentities($row_datosnombrestres['nombre_tres'], ENT_COMPAT, 'utf-8'); ?>" size="32"></td>
            </tr>
            <tr valign="baseline">
          ...
          <input type="hidden" name="idplan" value="<?php echo $row_datosnombrestres['idplan']; ?>">
        </form>
        </div>
      </fieldset>
      </div>
