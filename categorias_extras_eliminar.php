...
if ((isset($_GET['recordID'])) && ($_GET['recordID'] != "")) {
  $deleteSQL = sprintf("DELETE FROM categoria_extras WHERE idcategoria_extras=%s",
                       GetSQLValueString($_GET['recordID'], "int"));

  mysql_select_db($database_crenesoft, $crenesoft);
  $Result1 = mysql_query($deleteSQL, $crenesoft) or die(mysql_error());

  $deleteGoTo = "categorias_extras_ver.php";
...
if (isset($_GET["recordID"])) {
  $varCat_Recordset1 = $_GET["recordID"];
}
mysql_select_db($database_crenesoft, $crenesoft);
$query_Recordset1 = sprintf("SELECT * FROM categoria_extras WHERE categoria_extras.idcategoria = %s", GetSQLValueString($varCat_Recordset1, "int"));
