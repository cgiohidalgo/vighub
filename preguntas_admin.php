...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO respuestas (respuesta) VALUES (%s)",
  ...
  $insertGoTo = "preguntas_admin.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_crenesoft, $crenesoft);
$query_Recordset1 = sprintf("SELECT * FROM preguntas WHERE preguntas.id = %s", GetSQLValueString($varUsuario_Recordset1, "int"));
...
mysql_select_db($database_crenesoft, $crenesoft);
$query_todos = "SELECT * FROM preguntas ORDER BY preguntas.autor";
$todos = mysql_query($query_todos, $crenesoft) or die(mysql_error());
$row_todos = mysql_fetch_assoc($todos);
$totalRows_todos = mysql_num_rows($todos);

$maxRows_respuesta = 10;
$pageNum_respuesta = 0;
if (isset($_GET['pageNum_respuesta'])) {
  $pageNum_respuesta = $_GET['pageNum_respuesta'];
}
$startRow_respuesta = $pageNum_respuesta * $maxRows_respuesta;

mysql_select_db($database_crenesoft, $crenesoft);
$query_respuesta = "SELECT * FROM respuestas";
$query_limit_respuesta = sprintf("%s LIMIT %d, %d", $query_respuesta, $startRow_respuesta, $maxRows_respuesta);
$respuesta = mysql_query($query_limit_respuesta, $crenesoft) or die(mysql_error());
$row_respuesta = mysql_fetch_assoc($respuesta);

if (isset($_GET['totalRows_respuesta'])) {
  $totalRows_respuesta = $_GET['totalRows_respuesta'];
} else {
  $all_respuesta = mysql_query($query_respuesta);
  $totalRows_respuesta = mysql_num_rows($all_respuesta);
}
$totalPages_respuesta = ceil($totalRows_respuesta/$maxRows_respuesta)-1;
?>
<!doctype html>
		...
		<fieldset class="well">
			...
			<div class="span4">
              <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
                <table align="center">
                  <tr valign="baseline">
                    <td nowrap align="right" valign="top">Respuesta:</td>
                    <td><textarea name="respuesta" cols="60" class="message" required placeholder="Aqui va tu repuesta" rows="9"></textarea></td>
                  </tr>
                  <tr valign="baseline">
                    <td nowrap align="right">&nbsp;</td>
                    <td><input type="submit"class="btn btn-primary" value="Responder"></td>
                  </tr>
                </table>
...
<div class="span4">
					  <fieldset class="well well-small">
						  <legend>NOTA</legend>
						  <p class="alert alert-warning">Para responder una pregunta <strong>haz clic</strong> sobre ella y respóndela. ---<strong>¡GRACIAS!</strong>.</p>
			  </fieldset>
			  </div>
		  </div>	
		</fieldset>		
	...
	<div id="datos_usuario" class="span5">
		...
		<!--==============================================================-->
            ...
            <?php if(empty($row_todos)): ?>
             <p class="no_answer"> Aun no hay preguntas!!</p>
        ...
        <p class="no_answer">&nbsp;</p>
			 <?php else: ?>
              ...
              <?php do { ?>
                <table width="123%" border="1" cellspacing="2" cellpadding="1">
                  <tr >
                    <th  width="55%" bgcolor="#00CCCC"    align="justify"   scope="row">Respuesta:<a href="preguntas_admin.php"><?php echo $row_todos['pregunta']; ?></a></th>
                    <th bgcolor="#00CCCC" width="37%"  align="center"  scope="row"><?php echo $row_Recordset1['autor']; ?></th>
                   </tr> 
                  </table>
