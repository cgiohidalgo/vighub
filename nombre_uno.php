...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE planes SET nombre_uno=%s WHERE idplan=%s",
...
$varUsuario_datosnombres = "0";
if (isset($_SESSION["MM_Idusuario"])) {
  $varUsuario_datosnombres = $_SESSION["MM_Idusuario"];
}
mysql_select_db($database_crenesoft, $crenesoft);
$query_datosnombres = sprintf("SELECT * FROM planes WHERE planes.idplan = %s", GetSQLValueString($varUsuario_datosnombres, "int"));
$datosnombres = mysql_query($query_datosnombres, $crenesoft) or die(mysql_error());
$row_datosnombres = mysql_fetch_assoc($datosnombres);
$totalRows_datosnombres = mysql_num_rows($datosnombres);$varUsuario_datosnombres = "0";
if (isset($_SESSION["MM_idusuario"])) {
  $varUsuario_datosnombres = $_SESSION["MM_idusuario"];
}
mysql_select_db($database_crenesoft, $crenesoft);
...
$datosnombres = mysql_query($query_datosnombres, $crenesoft) or die(mysql_error());
$row_datosnombres = mysql_fetch_assoc($datosnombres);
$totalRows_datosnombres = mysql_num_rows($datosnombres);
?>
<!doctype html>
        ...
        <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
          <table align="center">
            <tr valign="baseline">
              ...
              <td><input type="text" required placeholder="Escribe el nombre del autor" title="ej: Giovanny Hidalgo Suarez" name="nombre_uno" value="<?php echo htmlentities($row_datosnombres['nombre_uno'], ENT_COMPAT, 'utf-8'); ?>" size="32"></td>
            </tr>
            <tr valign="baseline">
              
              <td><input type="submit" class="btn btn-primary" value="Continuar"></td>
            </tr>
          </table>
           </div>
           </fieldset>
          ...
          <input type="hidden" name="idplan" value="<?php echo $row_datosnombres['idplan']; ?>">
        </form>
         </div>
      </fieldset>
      </div> 
