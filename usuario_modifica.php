...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE usuarios SET grupo=%s, email=%s, password=%s, imagen=%s WHERE idusuario=%s",
...
$varUsuario_consultadatos = "0";
if (isset($_SESSION["MM_idusuario"])) {
  $varUsuario_consultadatos = $_SESSION["MM_idusuario"];
}
mysql_select_db($database_crenesoft, $crenesoft);
$query_consultadatos = sprintf("SELECT * FROM usuarios WHERE usuarios.idusuario = %s", GetSQLValueString($varUsuario_consultadatos, "int"));
$consultadatos = mysql_query($query_consultadatos, $crenesoft) or die(mysql_error());
$row_consultadatos = mysql_fetch_assoc($consultadatos);
$totalRows_consultadatos = mysql_num_rows($consultadatos);
?>
<!doctype html>
  ...
  <div id="contenedor_formulario" class="well">
      ...
      <legend align="center">¡Categorias!</legend>
		...
		<fieldset class="well">
			...
			<!--==============================================================-->
			<legend align="center">Editar categorias</legend>
			<div align="center" class="span4">     

<form method="post"  name="form1" action="<?php echo $editFormAction; ?>">
  <input type="text" placeholder="Nombre de la empresa" name="grupo" value="<?php echo htmlentities($row_consultadatos['grupo'], ENT_COMPAT, 'utf-8'); ?>" size="32"><input type="text" placeholder="Correo electronico" name="email" value="<?php echo htmlentities($row_consultadatos['email'], ENT_COMPAT, 'utf-8'); ?>" size="32"><input type="password" placeholder="conraseña" name="password" value="<?php echo htmlentities($row_consultadatos['password'], ENT_COMPAT, 'utf-8'); ?>" size="32">
...
?>
  ...
  <input type="text" placeholder="Selecciona  tu logo" name="imagen" value="<?php echo htmlentities($row_consultadatos['imagen'], ENT_COMPAT, 'utf-8'); ?>" size="32">
      
      <input type="button" name="button" onClick="javascript:cargarimagen();" class="btn btn-success" title="Si deseas saber como hacer un logo PNG con photoshop escribe en el link de ¿Dudas?" id="button" value="Subir logo">
  ...
  <input type="hidden" name="idusuario" value="<?php echo $row_consultadatos['idusuario']; ?>">
</form>
  </div>
      </fieldset>
      </div> 
